...
class ExecutorTests(unittest.TestCase):
    def setUp(self):
        ...
        self.sample_file = "data/raw/test_executor.xlsx"
    ...
    def test_multi_step_chains_previous_output(self):
        def fake_route(command, file_path):
            if command == "clean-duplicates":
                output_file = "outputs/test_cleaned.xlsx"
        ...
        self.assertEqual(result["results"][1]["input_file"], "outputs/test_cleaned.xlsx")
        self.assertTrue(os.path.exists(result["log_file"]))

    def test_missing_file_suggests_closest_match(self):
        result = executor.execute_plan(
            [{"command": "summarize", "file_path": "data/raw/test_executr.xlsx"}]
        )

        self.assertEqual(result["status"], "error")
        self.assertIn("Did you mean: data/raw/test_executor.xlsx?", result["message"])
